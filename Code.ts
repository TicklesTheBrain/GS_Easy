...
function boardReveal(){
    ...
    let player1Values = new SheetValues(SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Player1') as GoogleAppsScript.Spreadsheet.Sheet);
    let player2Values = new SheetValues(SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Player2') as GoogleAppsScript.Spreadsheet.Sheet);
...
function test(){
    let newValues = new SheetValues(SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Player1') as GoogleAppsScript.Spreadsheet.Sheet);
}
...
function boardNewRound(newGame = false){
    ...
    if (typeof(board.Round) != "number"){
        board.message = "Can't start a new round, this was the last one."
        boardValues.writeToFile();
        return
    ...
    let p1AvailableCards = allCardNames?.filter(e => !(e in player1.cardsDrawn));
    shuffleArray(p1AvailableCards as string[]);
    for (let i = 0; i<cardsToDraw; i++){
        player1.cardsDrawn.push(p1AvailableCards?.pop())
    }

    let p2AvailableCards = allCardNames?.filter(e => !(e in player2.cardsDrawn));
    ...
    //Clean any leftover cards if new game
...
function playerEndSetup(){
    ...
    let playerValues = new SheetValues(SpreadsheetApp.getActiveSheet())
    ...
    let currentCards = playerValues.createReaderAtLabel("Tableau")?.readArray(ValueMapDirection.Right, false).filter( element => {return element != "" && element != "None"}) as string[];
